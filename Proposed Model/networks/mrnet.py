...
class MRNet(nn.Module):
    ...
    def reduce(self, row_features, col_features, N, K0):
        ...
        final_row_features = self.apply_reduce(row1 + row2, row2 + row3, row3)
        ...
        final_col_features = self.apply_reduce(col1 + col2, col2 + col3, col3)
    ...
    def forward(self, x):
        ...
        a = self.final_low + self.final_mid 
        b = a + self.final_high
        c = self.final_high
        final = torch.cat([a, b, c], dim=1)
